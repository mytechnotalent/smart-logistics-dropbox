 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON COURIER smart logistics drop-box
 * status LED, courier-arrival button, locker servo, infrared decoder,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(DROPBOX_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(DROPBOX_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(dropbox_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(DROPBOX_OFF, 0, 0, 0);
    assert_show(DROPBOX_DENIED, 1, 0, 0);
    assert_show(DROPBOX_WAITING, 0, 1, 0);
    assert_show(DROPBOX_UNLOCKED, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_arrival_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(arrival_init());
    gpio_put(DROPBOX_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(arrival_pressed());
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_pressed());
}
 ...
 * @return void
 */
static void test_arrival_consume(void) {
    reset_pc();
    arrival_init();
    gpio_put(DROPBOX_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(arrival_consume_press());
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_consume_press());
    TEST_ASSERT_FALSE(arrival_consume_press());
}

/**
 * @brief Release the button and clear its consumed locker.
 *
 * @param void No parameters.
 * @return void
 */
static void arrival_release(void) {
    gpio_put(DROPBOX_BUTTON_PIN, true);
    arrival_consume_press();
}
 ...
 * @return void
 */
static void test_arrival_debounce(void) {
    reset_pc();
    arrival_init();
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_consume_press());
 ...
 * @return void
 */
static void test_arrival_debounce_elapsed(void) {
    reset_pc();
    arrival_init();
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_consume_press());
    arrival_release();
    mock_timer_set_us(mock_timer_now_us() + ARRIVAL_DEBOUNCE_US);
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_consume_press());
}

/**
 * @brief Verify arrival_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_arrival_reset(void) {
    reset_pc();
    arrival_init();
    gpio_put(DROPBOX_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(arrival_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(DROPBOX_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[DROPBOX_IR_PIN]);
}
 ...
 * @return void
 */
static void test_aead_dropbox_tag(void) {
    /**
     * @brief Declaration of ct.
     */
    uint8_t ct[32];
 ...
 * @return void
 */
static void test_aead_dropbox_ct(void) {
    /**
     * @brief Declaration of ct.
     */
    uint8_t ct[32];
 ...
 * @return void
 */
static void test_aead_dropbox_ad(void) {
    /**
     * @brief Declaration of ct.
     */
    uint8_t ct[32];
 ...
 * @return void
 */
static void assert_open_dropbox(void) {
    TEST_ASSERT_TRUE(seal_env());
...
/**
 * @brief Verify malformed, truncated, dropboxed, and short envelopes reject.
 *
 * @param void No parameters.
 * @return void
 */
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_arrival_pressed);
    RUN_TEST(test_arrival_consume);
    RUN_TEST(test_arrival_debounce);
    RUN_TEST(test_arrival_debounce_elapsed);
    RUN_TEST(test_arrival_reset);
}
...
static void run_aead_tests(void) {
    ...
    RUN_TEST(test_aead_dropbox_tag);
    RUN_TEST(test_aead_dropbox_ct);
    RUN_TEST(test_aead_dropbox_ad);
