#!/usr/bin/env python3
"""Inject a forged DROPBOX PACKAGE into a node to teach authentication.
...
PACKAGE command to the victim node. Against the naive node the forged
command moved the locker; against the hardened node the same attack fails
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the DROPBOX spoof tool"
    ) from exc
...
def _add_forge_args(parser):
    """Add the forged package and attack-mode options.
    ...
    parser.add_argument("--package", type=int, default=DEFAULT_PACKAGE,
                        help="Forged package in tenths of a degree Celsius")
...
def _command_body(seq, package):
    """Build a forged PACKAGE body of sequence, value, and random tag.
    ...
    package : int
        Forged package in tenths of a degree Celsius.
    ...
    return head + package.to_bytes(2, "little") + secrets.token_bytes(TAG_LEN)
...
def _frame(args):
    ...
    return _forged_envelope(_command_body(args.seq, args.package))


def main():
    """Inject the forged DROPBOX PACKAGE under the gateway address.
    ...
    print(f"Injected forged {args.mode} package {args.package} to node "
          f"0x{args.victim}")
