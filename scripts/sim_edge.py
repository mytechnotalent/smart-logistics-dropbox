#!/usr/bin/env python3
"""Simulate a real DROPBOX node from a laptop without a Pico.

sim_edge.py drives a USB-to-TTL radio with the same sealed request and
AT+SEND cadence as the RP2350 firmware. Each requested package is sealed
with the OPERATION COLD IRON field key so the mesh gateway accepts the
request and answers with a sealed dropbox command. Students can validate
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the DROPBOX edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed DROPBOX package request.
