...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("SMART LOGISTICS DROP-BOX", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION IRON COURIER", 9, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION IRON COURIER", 9, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ COVERT CHANNEL LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("SMART_LOGISTICS_SYSTEM", 5, colors["cyan"],
                banner.W // 2, 750)
    banner.text("LOCKER  COURIER  IR  DHT11", 4, colors["silver"],
                banner.W // 2, 900)
    banner.text("LOCKER LATCH  COURIER REMOTE", 3, colors["silver"],
                banner.W // 2, 990)
    banner.text("XCHACHA20-POLY1305  ARGON2ID", 4, colors["dim"],
                banner.W // 2, 1120)
    banner.text("COVERT CHANNEL", 4, colors["gold"],
                banner.W // 2, 1260)
    banner.text("EXFILTRATION  FAIL-LOCKED", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION IRON COURIER  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
